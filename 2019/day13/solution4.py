...
def game_player(opcodes: OpCodes):
    ...
    thread = threading.Thread(target=processor, name='Game', args=(opcodes, ), daemon=True)
    thread.start()
...
def main():
    ...
    for i in range(len(opcodes)-2):
        if opcodes[i] == 0 and opcodes[i+1] == 3 and opcodes[i+2] == 0:
            ball = i + 1
            break

    for r in range(ball - 17, ball + 18):
        opcodes[r] = 1  # fake wall
    score = game_player(opcodes=opcodes)
    print(score)
