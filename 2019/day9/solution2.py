...
def run_test_boost(opcodes):
    """
    Run the BOOSST system in test mode and get all the results.
    """
    input_queue = SimpleQueue()
    output_queue = SimpleQueue()

    processor = Processor(input_queue, output_queue)
    input_queue.put(2)  # sensor boost mode

    processor(opcodes)

    while not output_queue.empty():
        logging.info(output_queue.get())
