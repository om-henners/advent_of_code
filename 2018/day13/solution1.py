...
class Cart:
    ...
    def tick(self):
        ...
        if any([self == cart for cart in Cart.all_carts if cart is not self]):
            raise MineCartCollision


    # TODO: Write the movement and collision detection algorithm
...
print(f'Built track {track_layout.shape} and {len(minecarts)} mine carts')

ticks = 0
try:
    while True:

        for cart in sorted(minecarts):
            cart.tick()
            print(cart)
        ticks += 1
